		...
		for (int r = 1; r < state.length - 1; r++) {
			for (int c = 1; c < state[1].length - 1; c++) {
				if (state[r][c] == 1) {
					...
					/* imageR and imageC coorespond to the upper left pixel of the cell
					 * in the image.  The image's .setRGB() method starts at (x,y) as (0,0).
					int imageR = (r-1) * pixelsPerCell;
					int imageC = (c-1) * pixelsPerCell;
					for (int row = imageR; row < (imageR + pixelsPerCell); row++) {
						for (int col = imageC; col < (imageC + pixelsPerCell); col++) {
							setRGB(col, row, liveColor.getRGB());
						}
					} */
				}
			}
